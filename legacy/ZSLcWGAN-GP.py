...
for dir_path in [config["results_dir"], config["checkpoints_dir"], config["cache_dir"], f"{config['results_dir']}/fake", f"{config['results_dir']}/real", f"{config['results_dir']}/unseen_synthetic"]:
...
if split_file.exists():
    ...
    with open(split_file, "w") as f:
        json.dump({"seen": seen_classes.tolist(), "unseen": unseen_classes.tolist()}, f, indent=2)
...
# Create datasets
train_dataset = FilteredCIFAR100(root=config["data_root"], train=True, download=True, transform=transform_train, allowed_classes=seen_classes)
...
train_loader = DataLoader(train_subset, batch_size=config["batch_size"], shuffle=True, num_workers=num_workers, pin_memory=True, persistent_workers=True)
val_loader = DataLoader(val_subset, batch_size=config["batch_size"], shuffle=False, num_workers=num_workers, pin_memory=True, persistent_workers=True)
...
semantic_embeddings = create_semantic_embeddings(cifar100_classes, glove_embeddings, embedding_dim=config["embedding_dim"])

# Convert to tensors
all_semantic_embeddings = torch.tensor([semantic_embeddings[cls] for cls in range(config["num_classes"])], dtype=torch.float32, device=device)

seen_semantic_embeddings = torch.tensor([semantic_embeddings[cls] for cls in seen_classes], dtype=torch.float32, device=device)

unseen_semantic_embeddings = torch.tensor([semantic_embeddings[cls] for cls in unseen_classes], dtype=torch.float32, device=device)
...
class Generator(nn.Module):
    def __init__(self, nz=128, ngf=64, nc=3, semantic_dim=300, semantic_proj_dim=256):
        ...
        self.semantic_proj = nn.Sequential(
            nn.Linear(semantic_dim, semantic_proj_dim), nn.LeakyReLU(0.2, inplace=True), nn.Dropout(0.2), nn.Linear(semantic_proj_dim, semantic_proj_dim), nn.LeakyReLU(0.2, inplace=True)
        )
...
class Discriminator(nn.Module):
    def __init__(self, nc=3, ndf=64, semantic_dim=300):
        ...
        self.semantic_proj = nn.Sequential(spectral_norm(nn.Linear(semantic_dim, 256)), nn.LeakyReLU(0.2, inplace=True), spectral_norm(nn.Linear(256, 256)), nn.LeakyReLU(0.2, inplace=True))
...
def calculate_fid(real_dir, fake_dir):
    ...
    try:
        metrics = calculate_metrics(input1=real_dir, input2=fake_dir, cuda=torch.cuda.is_available(), fid=True, isc=True, kid=True, verbose=False)
        return {
...
class TrainingTracker:
    ...
    def update_metrics(self, metrics, epoch, netG, netD):
        ...
        if fid_score < self.best_fid:
            ...
            torch.save({"generator": netG.state_dict(), "discriminator": netD.state_dict(), "epoch": epoch, "metrics": metrics}, Path(config["checkpoints_dir"]) / "best_model.pth")
...
class ZSLClassifier(nn.Module):
    def __init__(self, num_classes):
        ...
        self.classifier = nn.Sequential(nn.Linear(512, 512), nn.BatchNorm1d(512), nn.LayerNorm(512), nn.ReLU(inplace=True), nn.Dropout(0.5), nn.Linear(512, num_classes))
...
netG = Generator(nz=config["nz"], ngf=config["ngf"], nc=config["nc"], semantic_dim=config["embedding_dim"], semantic_proj_dim=config["semantic_proj_dim"]).to(device)

netD = Discriminator(nc=config["nc"], ndf=config["ndf"], semantic_dim=config["embedding_dim"]).to(device)
...
for epoch in range(config["num_epochs"]):
    ...
    for real_images, labels in pbar:
        ...
        pbar.set_postfix({"G": f"{epoch_g_loss/epoch_batches:.4f}", "D": f"{epoch_d_loss/epoch_batches:.4f}", "W": f"{wasserstein_distance.item():.4f}"})
...
def evaluate_zsl_performance():
    ...
    # Load test data
    test_dataset = FilteredCIFAR100(root=config["data_root"], train=False, download=True, transform=transform_test, allowed_classes=unseen_classes)
    ...
    synthetic_dataset = SyntheticDataset(netG, num_samples=config["synthetic_samples_per_class"] * num_unseen, num_classes=num_unseen, semantic_embeddings=unseen_semantic_embeddings, nz=config["nz"])
    ...
    plt.axhline(y=mean_class_acc, color="red", linestyle="--", linewidth=2, label=f"Mean: {mean_class_acc:.2f}%")
    plt.axhline(y=top1_accuracy, color="green", linestyle="--", linewidth=2, label=f"Overall: {top1_accuracy:.2f}%")
...
"""
    ...
    ax7.text(0.1, 0.5, summary_text, fontsize=9, family="monospace", verticalalignment="center", bbox=dict(boxstyle="round", facecolor="wheat", alpha=0.3))
